...
def draw_cat_plot():
    ...
    df_cat = df_cat.groupby(['cardio', 'variable', 'value']).agg({'value': 'count'}).stack().reset_index().drop(columns=['level_3']).rename(columns={0: 'total'})
...
def draw_heat_map():
    ...
    sns.heatmap(corr, mask=mask, annot=True, linewidths=0.5, fmt='.1f')
